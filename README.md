...
| ID `kuro`（ID・パスワードタブ） | `@kuro` | 管理者。パスワードはオーナーが管理（リポジトリには bcrypt ハッシュのみ） |
...
| ブランド / UI | 白×オレンジ（`#FF6B1A`）のライトテーマ。ロゴは AP モノグラム + 青（`#2F6BFF`）のピリオド（`src/shared/ui/Logo.tsx`、`public/favicon.svg`、PWA アイコン） |
