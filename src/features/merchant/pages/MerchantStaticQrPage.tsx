...
export function MerchantStaticQrPage() {
  ...
  return (
    <div className="flex flex-col gap-4 px-4 pb-6">
      <p className="text-sm text-muted no-print">{t('merchant.qr.lead')}</p>
      <Button className="no-print" icon={<Printer className="h-4 w-4" />} onClick={printPage}>
        {t('merchant.qr.print')}
      </Button>

      <div className="print-sheet mx-auto flex w-full flex-col items-center gap-6 rounded-3xl border border-line bg-white px-6 py-10 text-center text-fg">
        <Logo size={22} />
