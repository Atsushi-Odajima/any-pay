...
import { Avatar, Card, EmptyState, LanguageToggle } from '@/shared/ui';
      ...
      <header className="flex items-center gap-2">
        <Link to="/settings/profile" className="flex min-w-0 items-center gap-2">
          ...
          <span className="truncate text-sm font-medium">
            {profile.data?.display_name ?? env.appName}
          </span>
        </Link>
      ...
      <section>
        ...
        <Card className="p-0">
          {recent.data && recent.data.length > 0 ? (
            ...
            <EmptyState title={t('home.empty')} description={t('home.emptySub')} />
          )}
        </Card>
      </section>
    </div>
