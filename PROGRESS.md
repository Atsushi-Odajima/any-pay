...
## 説明書（ユーザー / 加盟店 / 管理者、日英）
- 完了内容：「その他 → 説明書」に3種類の取扱説明書を追加（`/guide`、`/guide/:id`）。ユーザー向け（ログイン〜支払い3方式・クーポン・送金・割り勘・履歴・PIN・出金・エラー対処）、加盟店向け（店舗登録〜受付3方式・明細・返金・クーポン作成・出金・制限・エラー対処）、管理者向け（台帳の突合・全店共通クーポン・デモアカウント・ユーザー管理・デプロイ・セキュリティ方針・設定値・Stripe・既知の事象）。表示言語の切替に連動して日英が切り替わる。目次アンカー付き、右上「印刷」で A4 印刷 / PDF 保存（白黒スタイル）
- 仕組み：`features/guide/content/{types,ja,en}.ts` に構造化データ（p / steps / list / note / table）、`components/ManualBlock.tsx` で描画。ページは遅延読み込み
...
## 仕様書（外部エンジニア向け）と説明書の整理
- 完了内容：`docs/spec/` に日英の仕様書を追加。加盟店向け（`merchant-integration.{ja,en}.md`：接続情報・認証、店舗登録、QR ペイロード仕様、3 方式の決済フロー、クーポン適用時の金額、取引データ・Realtime、返金、出金・クーポン、冪等性・エラーコード・制限、責任分界、接続テスト手順）、決済代行向け（`payment-provider-integration.{ja,en}.md`：チャネル別要件（クレジット / 銀行連携 / 振込 / コンビニ / ウォレット）、支払い作成 API、承認画面、署名付き webhook（イベント・署名・再送・応答）、Any Pay 側の状態遷移、制限・非機能・セキュリティ要件、アダプタ、サンドボックスによる受け入れ基準、接続手順）
...
## ロゴ確定（AP モノグラム + 青いピリオド）
- 6 案（ファインダー・コイン / コイン・アロー / ウォレット・カード / リボン a / スキャン・フレーム / AP モノグラム）から AP モノグラムを採用。青丸の差し込みは 3 パターン（P の中 / A の頂点 / ピリオド）を比較し、小サイズでも視認できる「AP.」のピリオド（`#2F6BFF`）に決定
- `Logo.tsx`（`onLight` で白地用のオレンジ文字版）、`favicon.svg`、PWA アイコン、`docs/logo.png` を差し替え

## ロゴ改善（APY モノグラム）
- 「A P Y」の 3 文字に変更し、8° 前傾の均一ストローク（幅 9・丸い端）でスタイリッシュに。青丸を大きく（r 6.5 → 8）。直立 / 前傾 / 角端 / 横棒なしの 4 案を比較して前傾・丸端を採用
- `Logo.tsx`、`favicon.svg`、PWA アイコン、`docs/logo.png` を更新

## ロゴ改善 2（直立・幅広の APY）
- 前傾をやめて直立にし、各文字の縦横比を 1:1（A / Y = 高さ、P = 0.8 倍）、均一な字間、切り落とし端・尖った結合のローマン体構成に。青丸（r 8）は文字列の中央下に印章のように配置
- 中央下 / 右上（® 位置）/ 字間広め の 3 案を比較し、左右対称で端正な「中央下」を採用。`Logo.tsx`、`favicon.svg`、PWA アイコン、`docs/logo.png` を更新
